...
function AppContent() {
  ...
  const loadFavorites = async () => {
    ...
    if (data) {
      setFavorites(new Set(data.map(f => f.card_id)));
    }
  };
  ...
  const loadFavoriteCards = async () => {
    ...
    try {
      ...
      if (data) {
        setCards(data.map(f => f.card_data));
      }
    } catch (error) {
  ...
  const loadCollectionCards = async (collectionId: string) => {
    ...
    try {
      ...
      if (data) {
        setCards(data.map(c => c.card_data));
      }
    } catch (error) {
                ...
                {hasMore && (
                  <div className="flex justify-center mt-8">
                    <button
                      ...
                      disabled={loading}
                      className="px-8 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-semibold shadow-lg disabled:opacity-50"
                    >
